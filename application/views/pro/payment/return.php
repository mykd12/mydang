...
<head>
  ...
  function cancelTid() {
    ...
    var win = window.open('', 'OnLine',
      'scrollbars=no,status=no,toolbar=no,resizable=0,location=no,menu=no,width=600,height=400');
    win.focus();
        ...
        try {
            ...
            if (strcmp("0000", $_REQUEST["resultCode"]) == 0) {
                ...
                try {
                    ...
                    //echo "<pre>";
                    echo "<form id='frm' method='post' action='/DAO/payment'>";
					...
					if ((strcmp("0000", $resultMap["resultCode"]) == 0) && (strcmp($secureSignature, $resultMap["authSignature"]) == 0) ){	//결제보안 추가 2016-05-18
				       ...
				       * 여기에 가맹점 내부 DB에 결제 결과를 반영하는 관련 프로그램 코드를 구현한다.  
						 ...
						 [중요!] 승인내용에 이상이 없음을 확인한 뒤 가맹점 DB에 해당건이 정상처리 되었음을 반영함
								처리중 에러 발생시 망취소를 한다.
				       ******************************************************************************/
              $merchantData = explode(",", $merchantData);
              $key = $merchantData[0];
              $type = $merchantData[1];
              $psDate = $merchantData[2];
              $psCnt = $merchantData[3];
              $strtD = $merchantData[4];
              $slotKey = $merchantData[5];
              
              $price = $resultMap["TotPrice"];
              $method = $resultMap["payMethod"];
              $number1 = $resultMap["tid"];
              $number2 = $resultMap["MOID"];
              $date = $resultMap["applDate"];
              $time = $resultMap["applTime"];
              $code = $resultMap["resultCode"];
              echo "<input type='hidden' name='PST_IDX' value='".$slotKey."'/>";
              echo "<input type='hidden' name='USER_IDX' value='".$key."'/>";
              echo "<input type='hidden' name='USER_TYPE' value='".$type."'/>";
              echo "<input type='hidden' name='psDate' value='".$psDate."'/>";
              echo "<input type='hidden' name='psCnt' value='".$psCnt."'/>";
              echo "<input type='hidden' name='PMT_MONEY' value='".$price."'/>";
              echo "<input type='hidden' name='PMT_MEANS' value='".$method."'/>";
              echo "<input type='hidden' name='PMT_NUMBER1' value='".$number1."'/>";
              echo "<input type='hidden' name='PMT_NUMBER2' value='".$number2."'/>";
              echo "<input type='hidden' name='PMT_DATE' value='".$date."'/>";
              echo "<input type='hidden' name='PMT_TIME' value='".$time."'/>";
              echo "<input type='hidden' name='PMT_CODE' value='".$code."'/>";
              echo "<input type='hidden' name='strtD' value='".$strtD."'/>";
              if($method=='Card'){
                $cardNum = $resultMap["CARD_Num"];
                $CARD_Quota = $resultMap["CARD_Quota"];
                $CARD_Code = $resultMap["CARD_Code"];
                $CARD_CorpFlag = $resultMap["CARD_CorpFlag"];
                $CARD_CheckFlag = $resultMap["CARD_CheckFlag"];
                $CARD_PRTC_CODE = $resultMap["CARD_PRTC_CODE"];
                $CARD_Point = $resultMap["CARD_Point"];
                echo "<input type='hidden' name='PMT_CARD_NUMBER' value='".$cardNum."'/>";
                echo "<input type='hidden' name='PMT_INSTAL_PERIOD' value='".$CARD_Quota."'/>";
                echo "<input type='hidden' name='PMT_CARD_TYPE' value='".$CARD_Code."'/>";
                echo "<input type='hidden' name='PMT_CARD_QUOTA' value='".$CARD_Quota."'/>";
                echo "<input type='hidden' name='PMT_CARD_CORPFLAG' value='".$CARD_CorpFlag."'/>";
                echo "<input type='hidden' name='PMT_CARD_CHECKFLAG' value='".$CARD_CheckFlag."'/>";
                echo "<input type='hidden' name='PMT_CARD_PRTC' value='".$CARD_PRTC_CODE."'/>";
                echo "<input type='hidden' name='PMT_CARD_POINT' value='".$CARD_Point."'/>";
              }else if($method=='DirectBank'){
                $ACCT_BankCode = $resultMap["ACCT_BankCode"];
                $CSHR_Type = $resultMap["CSHR_Type"];
                $CSHR_ResultCode = $resultMap["CSHR_ResultCode"];
                echo "<input type='hidden' name='PMT_BANK_CODE' value='".$ACCT_BankCode."'/>";
                echo "<input type='hidden' name='PMT_RECEIPT_TYPE' value='".$CSHR_Type."'/>";
                echo "<input type='hidden' name='PMT_RECEIPT_CODE' value='".$CSHR_ResultCode."'/>";
                ...
                echo "<input type='hidden' name='PMT_VACT_NUM' value='".$VACT_Num."'/>";
                echo "<input type='hidden' name='PMT_VACT_BANKCODE' value='".$VACT_BankCode."'/>";
                echo "<input type='hidden' name='PMT_VACT_BANKNAME' value='".$vactBankName."'/>";
                echo "<input type='hidden' name='PMT_VACT_NAME' value='".$VACT_Name."'/>";
                echo "<input type='hidden' name='PMT_VACT_INPUTNAME' value='".$VACT_InputName."'/>";
                echo "<input type='hidden' name='PMT_VACT_DATE' value='".$VACT_Date."'/>";
                echo "<input type='hidden' name='PMT_VACT_TIME' value='".$VACT_Time."'/>";
              }
              // echo "<tr><th class='td01'><p>PRO_IDX</p></th>";
            ...
            if($resultMap["resultCode"]=='060780'){
              echo"<script>alert('잔액부족으로 결제가 되지 않았습니다.');history.go(-1);</script>";
            }else{
              echo"<script>alert('정상적으로 결제가 되지 않았습니다.');history.go(-1);</script>";
            }
            //echo"<script>alert('정상적으로 결제가 되지 않았습니다.');history.go(-1);</script>";
            //             echo "<tr><th class='td01'><p>거래 성공 여부</p></th>";
